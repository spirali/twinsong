import { CellId, EditorCell, KernelState, Notebook, NotebookDesc, OutputCell, OutputCellState, OutputValue, Run, RunId, TextOutputValue } from "./notebook";
...
export function stateReducer(state: State, action: StateAction): State {
    ...
    switch (action.type) {
        ...
        case 'fresh_run': {
            const runs = [...state.notebook.runs, {
                id: action.run_id,
            ...
            let runs = state.notebook.runs.map(r => {
                if (r.id == action.run_id) {
                    if (action.kernel_state == "ready" && r.output_cells.length > 0) {
                        ...
                        return { ...r, kernel_state: action.kernel_state, kernel_state_message: action.message };
                    }
                } else {
                    return r;
                }
                ...
                if (r.id == action.run_id) {
                    const output_cells = r.output_cells.map(c => {
                        if (c.id === action.cell_id) {
                            let values;
                            if (action.value !== "None" && 'Text' in (action.value as object) && c.values.length > 0 && c.values[c.values.length - 1] !== "None" && 'Text' in (c.values[c.values.length - 1] as object)) {
                                // Concatenate text values if both are Text type
                                values = [
                                    ...c.values.slice(0, -1),
                                    {
                                        Text: {
                                            value: (c.values[c.values.length - 1] as TextOutputValue).Text.value + (action.value as TextOutputValue).Text.value
                                        }
                                    }
                                ];
                            } else {

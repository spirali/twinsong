...
function checkIfLastLine(event: React.KeyboardEvent<HTMLDivElement> | React.KeyboardEvent<HTMLTextAreaElement>) {
  const textarea = event.target as HTMLTextAreaElement;
  const cursorPosition = textarea.selectionStart;
  return !textarea.value.substring(cursorPosition).includes("\n");
}

function checkIfFirstLine(event: React.KeyboardEvent<HTMLDivElement> | React.KeyboardEvent<HTMLTextAreaElement>) {
  const textarea = event.target as HTMLTextAreaElement;
  const cursorPosition = textarea.selectionStart;
      ...
      <Editor
        ...
        onKeyDown={(e) => {
          ...
          if (e.key == "ArrowUp" && prev_id && checkIfFirstLine(e)) {
            e.preventDefault();
            let textArea = document.getElementById(prev_id)?.getElementsByTagName("textarea")[0];
            if (textArea) {
              textArea.focus();
          ...
          if (e.key == "ArrowDown" && next_id && checkIfLastLine(e)) {
            e.preventDefault();
            let textArea = document.getElementById(next_id)?.getElementsByTagName("textarea")[0];
            if (textArea) {
              textArea.focus();

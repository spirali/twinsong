...
const TabButton = (props: {id: NotebookId | null, selectedId: NotebookId | null, label: JSX.Element}) => {
    ...
    <div className="w-screen fixed left-0 top-0">
      ...
      <div className="w-full border-b border-gray-200 bg-white">
        <div className="pl-4 flex">
            <TabButton id={null} selectedId={state.selected_notebook?.id || null} label={<NotebookTabs/>}/>
            {state.notebooks.map((notebook) => (
              <TabButton key={notebook.id} id={notebook.id} selectedId={state.selected_notebook?.id || null} label={<>{notebook.path}</>}/>
            ))}
        </div>
      </div>

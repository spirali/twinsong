...
import { AlertCircle, CheckCircle, Clock, PlayCircle, Info } from 'lucide-react';
import { OutputCell, OutputValue } from '../core/notebook';
import Editor from 'react-simple-code-editor';
import { highlight, languages } from 'prismjs/components/prism-core';
import 'prismjs/components/prism-python';

const OutputValueView: React.FC<{ value: OutputValue }> = (props: { value: OutputValue }) => {
  ...
  if ('Text' in value) {
    ...
    return <pre className="text-left">{value.Exception.message + "\n" + value.Exception.traceback}</pre>;
  }
  return null;
}

const OutputCellView: React.FC<{ cell: OutputCell }> = (props: { cell: OutputCell }) => {
  ...
  return (
    <div ref={ref} className="border border-gray-300 shadow-sm mb-2">
      ...
      {/* Content */}
      <div className={`p-1 ${props.cell.status === 'error' ? 'bg-red-50' : ''}`}>
